...
namespace Tayx.Graphy.Fps
{
    ...
    {
        public G_FpsManager(IntPtr obj) : base(obj) { }
        #region Variables -> Serialized Private

        [SerializeField] public    GameObject                  m_fpsGraphGameObject = null;
        [SerializeField] public    List<GameObject>            m_nonBasicTextGameObjects   = new List<GameObject>();
        ...
        private void Awake()
        {
            string[] bad = new string[] { "BG_", "fps_" };
            for (int i = 0; i < transform.childCount; i++)
                if (!bad.Any(x => transform.GetChild(i).name.StartsWith(x)))
                    m_nonBasicTextGameObjects.Add(transform.GetChild(i).gameObject);
            m_fpsGraphGameObject = transform.FindChild("fps_graph").gameObject;
            Init();
        }
